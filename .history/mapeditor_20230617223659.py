...
class Game:
    def __init__(self, scroll_speed, object_area_width, menu_start_x, menu_start_y, menu_item_width, menu_item_height, menu_items_per_row):
        ...
        self.dragging_group_objects = None  # Inicialize como uma lista vazia em vez de None
        self.dragging_objects_copy = None
    ...
    def handle_mouse_motion_event(self, event):
        ...
        if self.mouse_is_down and self.dragging_group_objects is not None:
            x, y = pygame.mouse.get_pos()  # Move esta linha para cá
    ...
    def handle_mouse_button_up_event(self, event):
        ...
        if self.dragging_group_objects is not None:
            x, y = pygame.mouse.get_pos()
            for obj, start_pos in zip(self.dragging_group_objects, self.dragging_start_pos):
                dx = x - start_pos[0]
                dy = y - start_pos[1]
        ...
        if self.adding_new_object and self.dragging_group_objects is not None:
            for clone, start_pos in zip(self.dragging_group_objects, self.dragging_start_pos):
                dx = x - start_pos[0] + self.mouse_offset_x
                dy = y - start_pos[1] + self.mouse_offset_y
                clone.move_to(clone.rect.x + dx, clone.rect.y + dy)
                self.mouse_offset_x = dx  # Atualize o self.mouse_offset_x
                self.mouse_offset_y = dy  # Atualize o self.mouse_offset_y
                self.game_objects.append(clone)
            self.adding_new_object = False
            self.dragging_group_objects = None
            self.dragging_from_menu = False
        ...
        if not object_clicked:
            self.dragging_group_objects = None  # Limpa o objeto arrastado no final
    ...
    def handle_copy_event(self, x, y):
        ...
        if self.selected_group_objects:  # Verifica se a lista de objetos selecionados não está vazia
            ...
            self.mouse_offset_x = mouse_offset_x  # Atualize o self.mouse_offset_x
            self.mouse_offset_y = mouse_offset_y  # Atualize o self.mouse_offset_y
    ...
    def handle_object_drag(self, x, y):
        if self.dragging_group_objects is not None and self.dragging_start_pos is not None:
            ...
            self.dragging_start_pos = [(obj.rect.x, obj.rect.y) for obj in self.dragging_group_objects]


    def check_menu_click(self, x, y):
        if self.dragging_group_objects is None:
    ...
    def set_dragging_objects(self, clones, x, y):
        print("dragging_group_objects foi chamado")
        self.dragging_group_objects = clones
        self.dragging_start_pos = [(x, y) for _ in clones]     
        print(self.dragging_group_objects)  # Adicione esta linha    


    def set_dragging_object_depth(self):
        if self.dragging_group_objects is not None:  # Verifica se self.dragging_group_objects não é None
            for obj in self.dragging_group_objects:  # Precisamos fazer a verificação e a mudança para cada objeto na lista
                if self.game_objects:
    ...
    def run(self):
        while True:
                ...
                # Desenha todos os objetos, exceto os que estão sendo arrastados em grupo
                for game_object in sorted(self.game_objects, key=lambda obj: obj.z):
                    if self.dragging_group_objects is None or game_object not in self.dragging_group_objects:
                ...
                if self.dragging_group_objects is not None and self.dragging_start_pos is not None:
                    ...
                    for obj, start_pos in zip(self.dragging_group_objects, self.dragging_start_pos):
                        dx = x - start_pos[0]
                        dy = y - start_pos[1]
                        clone = self.copy_object(obj)  # Cria uma cópia do objeto arrastado
                        clone.move_to(clone.rect.x + dx, clone.rect.y + dy)  # Move a cópia para a nova posição
                        clone.draw(screen)  # Desenha a cópia na tela
                        ...
                        # Atualiza as posições dos objetos originais correspondentes
                        for original_obj in self.original_objects:
                            if original_obj in self.game_objects:
                                index = self.game_objects.index(original_obj)
                                self.game_objects[index].move_to(clone.rect.x + dx, clone.rect.y + dy)
