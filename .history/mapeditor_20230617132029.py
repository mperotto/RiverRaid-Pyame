...
class Game:
    ...
    def handle_mouse_motion_event(self, event):
        if self.dragging_objects is not None:
            dx = event.pos[0] - self.dragging_start_pos[0]
            dy = event.pos[1] - self.dragging_start_pos[1]
            for obj in self.dragging_objects:
                obj.move_to(obj.rect.x + dx, obj.rect.y + dy)
            self.dragging_start_pos = event.pos
    ...
    def handle_mouse_button_down_event(self, event):
        ...
        if self.selected_group_objects:
            self.dragging_group_objects = self.selected_group_objects.copy()
