...
class Game:
    ...
    def handle_mouse_button_up_event(self, event):
        ...
        if self.adding_new_object and self.dragging_group_objects is not None:
            clone = self.dragging_group_objects[0]  # Obtém o objeto clonado
            clone.move_to(x - self.mouse_offset_x, y - self.mouse_offset_y)  # Define a nova posição do objeto clonado
            self.adding_new_object = False  # Define a flag de adição de novo objeto como False
