...
class Game:
        ...
        if event.key == pygame.K_DELETE:
            ...
            if self.selected_group_objects:  # Verifica se a lista de objetos selecionados não está vazia
                for game_object in self.selected_group_objects:
                     self.game_objects.remove(self.game_object)

                
        elif event.key == pygame.K_PAGEDOWN:
            if self.selected_group_objects:  # Verifica se a lista de objetos selecionados não está vazia
                for game_object in self.selected_group_objects:
                     self.game_object.z -= 1
    ...
    def handle_mouse_button_down_event(self, event):
        ...
        if self.selected_group_objects:
                self.dragging_group_objects = [obj.copy() for obj in self.selected_group_objects]  # Faz uma cópia superficial de cada objeto
    ...
    def handle_mouse_button_up_event(self, event):
        if self.dragging_group_objects is not None:
            for i, obj in enumerate(self.selected_group_objects):
                obj.x = self.dragging_group_objects[i].x
                obj.y = self.dragging_group_objects[i].y        
        self.dragging_objects = None
        self.dragging_group_objects = None
                ...
                if selection_rectangle.colliderect(game_object.rect):
                    if game_object not in self.selected_group_objects and game_object not in self.cloned_objects:  
                        ...
                        if pygame.key.get_mods() & pygame.KMOD_CTRL and not self.copy_started:
                            ...
                            self.cloned_objects.append(clone)
                            self.original_objects.append(game_object)
        elif self.dragging_objects is not None:
            dx = x - self.dragging_start_pos[0]
            dy = y - self.dragging_start_pos[1]

            for obj in self.dragging_objects:
        ...
        for game_object in self.game_objects:
            if game_object.rect.collidepoint(x, y):
                ...
                if game_object not in self.selected_group_objects:
                    ...
                    # Adicione o código para desenhar um quadrado vermelho em torno do objeto aqui
                break
        if not object_clicked:
            self.selected_group_objects = []
            self.selection_rectangle_start = (x, y)
        ...
        for i, (char, cls) in enumerate(char_to_class.items()):
            ...
            if rect.collidepoint(mouse_x, mouse_y):
                if self.game_objects:
                    ...
                    z = 1
                self.dragging_objects = cls(x, y, sheet_image, z)  # Add z here if your class accepts it
                
                self.mouse_offset_x = mouse_x - self.dragging_objects.rect.x
                self.mouse_offset_y = mouse_y - self.dragging_objects.rect.y
    ...
    def set_dragging_object_depth(self):
        if self.dragging_objects is not None:  # Verifica se self.dragging_object não é None
            if self.game_objects:
                self.original_z = self.dragging_objects.z
                self.dragging_objects.z = max(obj.z for obj in self.game_objects) + 1
            else:
                self.original_z = self.dragging_object.z
                self.dragging_objects.z = 1
    ...
    def run(self):
        while True:
                ...
                if self.dragging_group_objects is not None and self.dragging_start_pos is not None:
                    ...
                    dx = x - self.dragging_start_pos[0]
                    dy = y - self.dragging_start_pos[1]
                    for obj in self.dragging_group_objects:
                        obj.move_to(obj.rect.x + dx, obj.rect.y + dy)
                    self.dragging_start_pos = (x, y)  
